...
export default function RoomPageClient({ roomId }: RoomPageClientProps) {
  ...
  const [emojiPickerTab, setEmojiPickerTab] = useState<'emojis' | 'kaomojis'>('emojis');
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      const toggleBtn = document.querySelector('.composer-left-btn');
      if (
    ...
    <div className="chat-layout">
      ...
      <section className="chat-main">
          ...
          return (
            <div className="reply-preview-bar-clay">
              ...
              <button 
                ...
                title="Cancel reply"
              >
                <HugeiconsIcon icon={Cancel01IconComp} size={14} />
              </button>
            </div>
          );
        })()}
        ...
        <div className="composer-panel-clay" style={{ position: 'relative' }}>
          {showEmojiPicker && (
            <div className="emoji-picker-popover-clay" ref={emojiPickerRef} onClick={(e) => e.stopPropagation()}>
              <div className="emoji-picker-tabs">
                <button 
                  type="button" 
                  className={`emoji-tab-btn ${emojiPickerTab === 'emojis' ? 'active' : ''}`}
                ...
                <button 
                  type="button" 
                  className={`emoji-tab-btn ${emojiPickerTab === 'kaomojis' ? 'active' : ''}`}
                  onClick={() => setEmojiPickerTab('kaomojis')}
                >
                  Kaomojis
                </button>
              </div>
              <div className="emoji-picker-content">
                {emojiPickerTab === 'emojis' ? (
                  <div className="emoji-grid">
                    {EMOTICONS_LIST.map((emoji, index) => (
                      <button
                        key={`${emoji}-${index}`}
                        type="button"
                        className="emoji-picker-btn-item"
                  ...
                  <div className="kaomoji-list">
                    {KAOMOJIS_LIST.map((kaomoji, index) => (
                      <button
                        key={`${kaomoji}-${index}`}
                        type="button"
                        className="kaomoji-picker-btn-item"
                        onClick={() => handleInsertEmoji(kaomoji)}
                      >
                        {kaomoji}
                      </button>
                    ))}
          ...
          <button 
            type="button" 
            className={`composer-left-btn ${showEmojiPicker ? 'active' : ''}`}
